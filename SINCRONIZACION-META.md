...
El script trae el año en curso **y el anterior**, y los deja en `datos` dentro del
JSON. Cuando hay más de un año con datos, el informe muestra un selector de año a
la izquierda del filtro de período. Sin eso, el 1 de enero el informe habría
arrancado vacío y el año anterior habría desaparecido de la vista.

Cuántos años atrás se traen se controla con la variable `META_ANIOS_ATRAS` (por
defecto 1). Los años sin ninguna entrega no se incluyen, así que el selector no
aparece hasta que haya realmente dos años con datos.

Los datos de CRM —Funnel, Nuevas ONs, ONs Concretadas— están cargados a mano y
corresponden a 2026, según la constante `CRM_ANIO` en `comercial.html`. Cuando se
mira otro año, esas secciones dejan de filtrarse por mes, se muestran completas y
lo aclaran en su subtítulo: no tendría sentido cruzar meses de un año con ONs de
otro. Al actualizar los datos de CRM hay que actualizar también `CRM_ANIO`.
...
**1. Generar el token.** Lo ideal es un token de **usuario del sistema** de
Business Manager con permiso `ads_read`, porque no vence. Requiere que la cuenta
publicitaria esté dentro de un portafolio comercial. Si no lo está —el caso
actual—, se usa un token de usuario de larga duración generado desde una app de
Meta, que **vence a los sesenta días** y hay que renovar a mano.
...
Opcionalmente podés definir las variables `META_AD_ACCOUNT_ID`,
`META_API_VERSION` y `META_ANIOS_ATRAS` en la pestaña Variables de la misma
pantalla. Si no las definís, se usan `370211010327709`, `v25.0` y `1`.
